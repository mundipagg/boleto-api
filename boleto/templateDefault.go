...
const boletoPropostaLayoutFormDefault = `
{{define "boletoForm"}}
<div class="document">
    <table cellspacing="0" cellpadding="0">
        <tbody>
        ...
        <tbody>
            ...
            <tr>
                <td colspan="5"><span class="title">Nome do Beneficiário / CNPJ / CPF / Endereço:</span>
                    ...
                    <span class="text" id="recipient_document" style="margin-left:5em;width:25%"><b>{{.View.Boleto.Recipient.Document.Type}}</b> {{fmtDoc .View.Boleto.Recipient.Document}}</span>
                    <br>
                    <span class="text" id="recipient_address">{{.View.Boleto.Recipient.Address.Street}}, 
                    {{.View.Boleto.Recipient.Address.Number}} {{.View.Boleto.Recipient.Address.Complement}}  
                    {{.View.Boleto.Recipient.Address.District}}, 
...
func getTemplateDefault(bt string) (string, string) {
	if bt == "BDP" {
		return templateBoletoDefault, boletoPropostaLayoutFormDefault
	}
